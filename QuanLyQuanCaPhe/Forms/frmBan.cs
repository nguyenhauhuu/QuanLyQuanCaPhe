...
namespace QuanLyQuanCaPhe.Forms
{
    public partial class frmBan : Form
    {
                ...
                try
                {
                    using (XLWorkbook workbook = new XLWorkbook(openFileDialog.FileName))
                    {
                        ...
                        if (table.Rows.Count > 0)
                        {
                            ...
                            {
                                Ban ban = new Ban();
                                ban.TenBan = r["TenBan"].ToString() ?? "N/A";
                                context.Ban.Add(ban);
                            }
                            context.SaveChanges();
        ...
        private void btnXuat_Click(object sender, EventArgs e)
        {
            ...
            saveFileDialog.FileName = "Ban_" + DateTime.Now.ToShortDateString().Replace("/", "_") + ".xlsx";
                ...
                try
                {
                    ...
                    if (ban != null)
                    {
                        foreach (var p in ban)
                            table.Rows.Add(p.ID, p.TenBan);
                    }

                    // Gán bảng tạm vào Sheet 1 của tập tin Excel 
                    using (XLWorkbook wb = new XLWorkbook())
                    {
                        var sheet = wb.Worksheets.Add(table, "Ban");
